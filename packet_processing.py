...
protocol_mapping_l4 = {
    ...
    41: 'IPv6',
    58: 'IPv6-ICMP',
    84: 'TTP', # Transaction Transfer Protocol
...
def packet_rate_final(folder, mac_address, window=None):
    ...
    mac_address2 = mac_address.replace(':','')
    ...
    output_file_name = path + mac_address2 + "_rate_by_protocol_" +  last_folder + "_" + str(window) + ".log"
...
def bytes_rate_final_fixed_window(folder, mac_address, window=None):
    ...
    output_file_name = path + mac_address2 + "_bytes_rate_by_protocol_" +  last_folder + "_" + str(window) + ".log"
...
def protocol(folder):
    ...
    old_mac_address = mac_address_fixer(folder[len(folder)-12:len(folder)])
    mac_address = ""
...
def queryMAP(folder):
    """
    This method creates a table for the start and end time of every HTTP/HTTPS query
    Args:
        folder: folder containing pcap files
    """
    request = [] #["IP","First Packet","Final Packet","Port Numbers","Packets"]
    old_mac_address = folder[len(folder)-12:len(folder)]
    mac_address = ""
    # The MAC address for the device
    for i in range(len(old_mac_address)//2 - 1):
    ...
    output_file_name = "./results/ipQueries/ipQueries" + old_mac_address + ".csv"
    ...
    for filename in files:
        ...
        for ether_pkt in packets:
            ...
            # TCP
            if TCP in ether_pkt:
                port = ether_pkt[TCP].sport
                port = ether_pkt[TCP].dport
                # Outgoing
                if ether_pkt.src == mac_address:
                    savedIP = ether_pkt.dst
                    port = ether_pkt[TCP].dport
                # Incoming
                else:
                    savedIP = ether_pkt.src
                    port = ether_pkt[TCP].sport
                for item in request:
                    if item[0] == savedIP:
                        item[4] += 1
                        item[2] = ether_pkt.time
                        if port not in item[3]:
                            item[3].append(port)
                        break
                else:
                    request.append([savedIP,ether_pkt.time,ether_pkt.time,[port],1])
                        
            # UDP
            elif UDP in ether_pkt:
                sport = ether_pkt[UDP].sport
                dport = ether_pkt[UDP].dport
                # Outgoing
                if ether_pkt.src == mac_address:
                    savedIP = ether_pkt.dst
                # Incoming
            ...
            # Record packets while query are open
            else:
                active = 0

    out_file.write("IP|First Packet|Final Packet|Port Numbers|Packets\n")
    for item in request:
